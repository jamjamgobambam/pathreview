...
- Whether to apply this as global middleware (every route) vs. a per-route dependency (opt-in per endpoint) — a global approach is more thorough but riskier to add late in a project with existing untested routes; a targeted per-route approach is safer given the 4-6 hour estimate, starting with `/auth/login`.
- Existing project-wide mypy failures (103 errors on `main`, unrelated to this issue) may make `make check` noisy; need to confirm my new code doesn't introduce additional errors beyond what's already present, since CI likely runs the same check.
- **[Discovered during testing] Timestamp collision in `RateLimiter`:** `check_rate_limit` uses `time.time()` as both the score and the unique member of a Redis sorted set (`self.redis.zadd(key, {str(now): now})`). Two requests landing within the same clock tick produce an identical member string, which Redis silently deduplicates — undercounting real requests. This surfaced as flaky test results when firing 100+ requests in rapid succession. It's a pre-existing characteristic of `RateLimiter`, out of scope to fix for this issue, but worth flagging since it could affect accuracy under genuine high-throughput bursts (not just fast test loops) in production.
- **[Discovered during testing] Shared event loop for async tests:** the async DB engine in `core/database.py` is a module-level singleton bound to whichever event loop exists when first used. pytest-asyncio's default per-test loop scope tore that loop down between tests, causing a second async integration test to fail with "Event loop is closed." Fixed by setting `asyncio_default_fixture_loop_scope`/`asyncio_default_test_loop_scope` to `"session"` in `pyproject.toml` — confirmed this doesn't affect the existing unit test suite (53 pre-existing failures unchanged).