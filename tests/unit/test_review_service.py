...
class TestReviewService:
    ...
    def _setup_profile_query(
        self,
        mock_db_session: AsyncMock,
        profile_exists: bool = True,
    ) -> Mock | None:
        result = Mock()
        profile = Mock() if profile_exists else None
    ...
    async def test_create_review_returns_review_with_pending_status(
        ...
        self._setup_profile_query(mock_db_session, profile_exists=True)
    ...
    @pytest.mark.asyncio
    async def test_create_review_returns_none_when_profile_not_owned(
        self,
        mock_db_session: AsyncMock,
    ) -> None:
        """Test create_review returns None when profile does not belong to user."""
        profile_id = uuid4()
        user_id = uuid4()
    ...
    async def test_list_reviews_ordered_by_created_at(
        ...
        # List reviews issues a count query and a paginated select query
