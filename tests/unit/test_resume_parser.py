...
class TestResumeParser:
    ...
    def test_parse_invalid_content_type(self, parser: ResumeParser) -> None:
        ...
        with pytest.raises(ValueError) as exc_info:
            parser.parse(12345)  # type: ignore[arg-type]  # Invalid: integer, testing runtime check
    ...
    def test_detect_sections_with_leading_whitespace(self, parser: ResumeParser) -> None:
        """Regression test for #147: indented section headers should still be
        detected, since PDF text extraction commonly preserves per-line
        indentation from the original document layout."""
        text = (
            "\n    John Smith\n    john@example.com\n\n"
    ...
    def test_detect_sections_with_tabs_and_inconsistent_indentation(
        self, parser: ResumeParser
    ) -> None:
        """Headers indented with tabs, or at different depths within the same
        document, should both be detected."""
        text = "\tExperience:\nSenior Developer\n\n        Education:\nBS Computer Science\n"
