...
class TestPIIScrubber:
    ...
    def test_detect_parenthesized_phone_only(self, scrubber: PIIScrubber) -> None:
        """Regression test for #146: detect() must find a parenthesized
        phone number even when no other phone format is present in the
        text. Previously the phone_us regex required "-" or "." right
        after the closing paren, so a text containing only
        "(555) 123-4567" produced zero detections.
        """
        text = "Call me at (555) 123-4567"
        detected = scrubber.detect(text)
        ...
        assert "4567" in phone_detections[0]["value"]

    def test_scrub_parenthesized_and_dashed_phone_together(self, scrubber: PIIScrubber) -> None:
        """Regression test for #146: both formats in the same string
        must be redacted, not just the dashed one.
        """
        text = "Call me at (555) 123-4567 or 555-999-8888"
        scrubbed = scrubber.scrub(text)

        assert "(555) 123-4567" not in scrubbed
        assert "555-999-8888" not in scrubbed
        assert scrubbed.count("[REDACTED]") == 2

    def test_phone_us_space_separated_tradeoff(self, scrubber: PIIScrubber) -> None:
        """Documents an accepted trade-off from the #146 fix: widening
        the phone_us separator to accept whitespace (needed for formats
        like "(555) 123-4567" and "+1 555 123 4567") also means a bare
        3-3-4 digit sequence separated by spaces will be treated as a
        phone number, even if it isn't one. This is intentional -- for a
        PII scrubber, over-redaction is the safer failure mode.
        """
        text = "Order 123 456 7890 shipped yesterday"
        scrubbed = scrubber.scrub(text)
