...
class TestFaithfulnessChecker:
    ...
    def test_mixed_none_and_valid_text_chunks(self, checker):
        """Test context chunks mixing None text with valid strings."""
        feedback = "Has Python and AI skills"
        context_chunks = [
            {"text": "Strong Python skills"},
            {"text": None},
            {"text": "AI and machine learning expertise"},
        ]

        score = checker.check(feedback, context_chunks)

        # Should handle gracefully and still use the valid chunks
        assert isinstance(score, float)
        assert 0.0 <= score <= 1.0
