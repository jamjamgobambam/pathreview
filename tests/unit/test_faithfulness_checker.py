...
class TestFaithfulnessChecker:
    ...
    def test_common_words_filtered_in_overlap(self, checker):
        ...
        # Use sentences that ONLY share stop words ("the", "is", "in", "a")
        claim = "The bug is in a file"
        context = "The typo is in a script"

        supported = checker._is_supported(claim, context)

        # The meaningful words ("bug", "file" vs "typo", "script") do not overlap.
        # Stop words ("the", "is", "in", "a") are ignored.
        assert isinstance(supported, bool)
        assert supported is False
    ...
    def test_short_claims_retained_and_evaluated(self, checker):
        """Test that concise/short claims (e.g. 'Knows Rust.') are not discarded."""
        # This test ensures that short claims are not filtered out during extraction
        feedback = "Knows Rust."
        context_chunks = [{"text": "Rust systems programming expertise."}]

        claims = checker._extract_claims(feedback)
        assert "Knows Rust" in claims or "Knows Rust." in claims
