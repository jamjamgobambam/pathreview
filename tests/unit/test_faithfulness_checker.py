...
class TestFaithfulnessChecker:
    ...
    def test_mixed_none_and_valid_text_chunks(self, checker):
        """Test that a None text chunk doesn't break scoring when other chunks have valid text."""
        feedback = "Has strong Python programming skills."
        context_chunks = [
            {"text": None},
            {"text": "Strong Python programming experience shown in projects."},
        ]

        score = checker.check(feedback, context_chunks)

        # The valid chunk should still be used for support, despite the None chunk
        assert isinstance(score, float)
        assert score > 0.0
