...
class TestFaithfulnessChecker:
    ...
    def test_mixed_none_and_valid_chunk_text(self, checker):
        """Test that one chunk with None text doesn't zero out chunks with valid text."""
        feedback = "Has Python skills"
        context_chunks = [
        ...
        assert isinstance(score, float)
        # The valid chunk should still support the claim even though the
        # other chunk's text is None.
        assert score > 0.0
