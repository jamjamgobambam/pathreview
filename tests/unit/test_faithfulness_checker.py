...
class TestFaithfulnessChecker:
    ...
    def test_common_words_filtered_in_overlap(self, checker: FaithfulnessChecker) -> None:
        ...
        # "project" and "documented" overlap meaningfully; "the"/"is" are filtered as stop words
        assert supported is True
    ...
    # What I added (3 methods below, commented out one no longer needed)
    # def test_check_raises_when_chunk_text_is_none(self) -> None:
    #     checker = FaithfulnessChecker()
    #     with pytest.raises(AttributeError):
    #         checker.check("Some feedback text.", [{"text": None}])

    def test_check_handles_none_text_mixed_with_valid_chunks(self) -> None:
        """Test that a None-text chunk mixed with valid chunks doesn't crash and still scores
        using the valid chunk."""
        checker = FaithfulnessChecker()
        score = checker.check(
            "The essay demonstrates strong analytical thinking.",
            [
                {"text": "The essay demonstrates strong analytical thinking skills."},
                {"text": None},
            ],
        )
        assert isinstance(score, float)

    def test_check_handles_all_none_text_chunks(self) -> None:
        """Test that all-None-text context chunks don't crash and yield a score of 0.0."""
        checker = FaithfulnessChecker()
        score = checker.check(
            "The essay demonstrates strong analytical thinking.",
            [{"text": None}, {"text": None}],
