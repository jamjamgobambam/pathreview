...
class FaithfulnessChecker:
    ...
    def check(self, feedback: str, context_chunks: list[dict]) -> float:
        ...
        Args:
            ...
            context_chunks: Retrieved context chunks. A chunk whose "text" key
                is missing or None contributes nothing to the context and is
        ...
        # Concatenate context text. A chunk can carry "text" with an explicit
        # None, and .get only falls back to its default when the key is absent,
        # so `or ""` is needed too; str() keeps a non-string from raising.
