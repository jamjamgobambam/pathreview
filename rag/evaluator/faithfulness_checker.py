...
class FaithfulnessChecker:
    ...
    def check(self, feedback: str, context_chunks: list[dict]) -> float:
        ...
        # Concatenate context text. Use `or ""` rather than `.get(..., "")`
        # because a chunk can have the "text" key present with a None value
        # (e.g. a document that produced no extractable text upstream) --
        # dict.get()'s default only applies when the key is missing, not
        # when it's present with a falsy/None value.
