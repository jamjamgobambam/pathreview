...
class GitHubTool(BaseTool):
    ...
    def _has_tests(self, username: str, repo_name: str) -> bool:
        """Check if repository has test files or test configuration.
        ...
        Returns:
            True if test files/folders or test config exists
        """
        test_patterns = [
            "test",
            "tests",
        ...
        for pattern in test_patterns:
            url = f"{self.base_url}/repos/{username}/{repo_name}/contents/{pattern}"
            try:
                response = httpx.head(url, headers=headers, timeout=5.0)
